...
def get_predictions(experiment, labels):
    ...
    return x_test, y_test, y_pred, test_labels, pred_labels


def get_data(path, labels):
    x_train = np.array(pd.read_csv(os.path.join(path, 'X_train.csv'), header=None))
    y_train = np.array(pd.read_csv(os.path.join(path, 'y_train.csv'), header=None), dtype=int).squeeze()
    x_test = np.array(pd.read_csv(os.path.join(path, 'X_test.csv'), header=None))
    y_test = np.array(pd.read_csv(os.path.join(path, 'y_test.csv'), header=None), dtype=int).squeeze()
